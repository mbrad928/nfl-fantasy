...
- **Owner colors** (`playerColors` in the same doc, `{ownerId: "#hexcolor"}`) — each owner can click the dot next to their team name to pick their own color via the browser's native color picker. Falls back to the color in `data.js` until someone customizes it. No uniqueness is enforced — two owners can pick the exact same color.
